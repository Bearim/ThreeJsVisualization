...
import {getTowerLevels, getTowerLevelsWithEquipment} from "../../services/towersAPI";
...
class TowerViewer extends Component {
    ...
    componentDidMount() {
        ...
        getTowerLevels(this.tower.id).then(({data: levels}) => {
            getTowerLevelsWithEquipment(this.tower.id).then(({data: levelsWithEquipment}) => {
                const devices = levelsWithEquipment.flatMap(device => device.antennas);
                this.addExampleComponents(this.tower.height, levels, devices);
            });
        }, () => {
    ...
    configureScene = () => {
        ...
        this.renderer.setClearColor(0xaabbcc, 1);
    ...
    getTowerWidthAtTop = height => height / 500;
    ...
    createExampleDevice = (height, deviceInfo) => {
        ...
        const cylinderMaterial = new MeshLambertMaterial({color: this.getRandomColor()});
    ...
    tower = {
        id: '02b49cfc-c62c-44f9-872a-1f0cf6d23747',
        height: 180
    };

    // please don't remove these data, as part of our team works without API
